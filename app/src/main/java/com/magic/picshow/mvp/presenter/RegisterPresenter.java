...
public class RegisterPresenter extends BasePresenter<RegisterContract.Model, RegisterContract.View> {
    ...
    public void requestRegist(String account, String password, String code) {
        mModel.register(account, password, code).subscribeOn(Schedulers.io())
                .retryWhen(new RetryWithDelay(3, 2))//遇到错误时重试,第一个参数为重试几次,第二个参数为重试的间隔
                .doOnSubscribe(new Action0() {
                    @Override
                    public void call() {
                        Log.e("Regist", "doOnSubscribe");
                    }
                ...
                .subscribe(new ErrorHandleSubscriber<BaseJson<LoginInfo>>(mErrorHandler) {
                    ...
                    public void onNext(BaseJson<LoginInfo> response) {
                        if (response.isSuccess()) {
                            ...
                            mRootView.registerSuccess(mLoginInfo);
                            Log.e("Regist", "onNext = " + mLoginInfo.toString());
                        } else {
    ...
    public void requestUpdatePassword(String account, String password, String code) {
        mModel.register(account, password, code).subscribeOn(Schedulers.io())
                .retryWhen(new RetryWithDelay(3, 2))//遇到错误时重试,第一个参数为重试几次,第二个参数为重试的间隔
                .doOnSubscribe(new Action0() {
                    @Override
                    public void call() {
                        Log.e("requestUpdatePassword", "doOnSubscribe");
                    }
                }).subscribeOn(AndroidSchedulers.mainThread())
                .observeOn(AndroidSchedulers.mainThread())
                .doAfterTerminate(new Action0() {
                    @Override
                    public void call() {
                        Log.e("requestUpdatePassword", "doAfterTerminate");
                    }
                })
                .compose(RxUtils.<BaseJson<LoginInfo>>bindToLifecycle(mRootView))//使用RXlifecycle,使subscription和activity一起销毁
                .subscribe(new ErrorHandleSubscriber<BaseJson<LoginInfo>>(mErrorHandler) {
                    ...
                    public void onNext(BaseJson<LoginInfo> response) {
                        if (response.isSuccess()) {
                            ...
                            mLoginInfo.saveLoginInfo();
                            mRootView.registerSuccess(mLoginInfo);
                            Log.e("requestUpdatePassword", "onNext = " + mLoginInfo.toString());
                        } else {
